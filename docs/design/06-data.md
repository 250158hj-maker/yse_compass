...
> **`glossary.md` の定義を転記しない。** 各テーブルに「対応する語彙：`../glossary.md` §4 提出」と書き、定義本体は参照で済ませる（`README.md` §4-1）。
...
**書けない。障害が最も多く、かつ本設計書のクリティカルパス。**

### 先に解く必要がある正典の矛盾

| 障害 | 内容 |
| --- | --- |
| **A-1** | H-5「発表順は発表エンティティの整数属性・決定操作の画面は作らない」と `../requirements.md` §3-10「先生は発表順を編集できる」が**両方とも確度=確定で矛盾**。**発表順をどのテーブルが持つかが決まらない** |
| **A-3** | 概要は「1 チーム 1 件」（`glossary.md` §5）か「4 発表会すべてにある」（`requirements.md` §3-1）か。**概要テーブルの粒度が決まらない** |

### ER 直結の未決

| 障害 | 内容 | 先行方針 |
| --- | --- | --- |
| **H-2** | 資料枠が「提出方式」の属性を持たない | 両方書いて差を見る |
| **H-6** | 提出日時が単一だと期限内提出が遅延に化ける | **2 値で組む**（初回提出日時＋最終更新日時） |
| **H-3** | クラスが誰にぶら下がるか・誰が登録するかが未定義 | — |
| **H-1** | 「概要集」が資料枠と生成物の二重の意味 | — |
| **#7** | テンプレートの扱い | — |
...
| 定番の資料枠 | 概要集／プレゼン資料／概要設計書／詳細設計書／試作品一式／完成作品一式 | 企画書／補足資料／設計書／ER図／発表スライド／デモ動画／成果報告書（**全不一致・概要枠が無い**） |
...
**この章が開くと 03・04・05・07 が一斉に開く**（`README.md` §5-4）。逆に言えば、ここを開けない限り設計書は完成しない。
