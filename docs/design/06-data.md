...
> **更新のしかた**：上書き
> **主担当**：蒲山
> **最終更新**：2026-09-21（水戸・**採番の衝突を解消**＝本 PR が起票した **H-23 → H-26**／**H-24 → H-27**。同 ID を PR #28（H-23・2026-09-17 起票）と PR #31（H-24・2026-09-20 起票）が先に使っていた。先着優先で本 PR 側を動かした。**問い・先行方針・担当は変更していない**）／2026-09-21（蒲山・水戸のレビュー（PR #23）を受けて修正。ER 図のカーディナリティ2件の訂正・6-7 を「手書き SQL が要る項目の一覧」へ再構成・H-26／H-27 の起票と先行方針での反映・文書内のレビュー指摘記号の削除・参照形式の統一）／2026-09-11（蒲山・6-1〜6-6 の初稿。H-20 を起票し時刻枠は先行方針で除外。H-21・H-22 の決着を反映し、`teams.number` を置かず `works` の公開許可を3値にした）
...
erDiagram
    ...
    MATERIAL_SLOT {
        ...
        string slot_type
        timestamp deadline "nullable・H-27未決"
        boolean is_required
...
| deadline | timestamp | NULL | — | — | **ソフトデッドライン**（超過しても提出可・2026-07-26）。**NULL 可（H-27・未決）**：未設定＝締切なし・遅延判定の対象外。年度セットアップは日程未定のまま資料枠を生成する（`../requirements.md` §3-1）ため、NOT NULL・既定値なしでは生成できない |
| is_required | boolean | NOT NULL | — | — | 必須／任意の2値（2026-08-18）。**H-14（試作品一式）は2値のまま先行** |
| template_url | text | NULL | — | — | テンプレート参照（任意・2026-07-24） |

> **`deadline` は H-27 の先行方針で組んでいる。** **前提**：締切が未設定の資料枠を許す。**決着時に変わる箇所**：`deadline` の NULL 可否・既定値／6-6 の遅延の導出式（NULL の扱い）。**未決 ID**：H-27。
...
| 遅延（提出状況一覧・§3-2） | `material_slots.deadline IS NOT NULL` かつ `submissions.first_submitted_at > material_slots.deadline`（概要枠は `summaries.first_submitted_at` と `slot_type='概要'` の `material_slots.deadline`）。**締切が未設定の枠は遅延判定の対象外**（H-27 先行方針） | **H-6 先行方針**（`../open-questions.md` §2・**未決**）。**最終更新日時ではなく初回提出日時で判定する**ことで、期限内提出が差し替えで遅延に化ける問題を避ける |
...
> **遅延の導出（H-6）と、締切の NULL の扱い（H-27）は、未決の先行方針で組んでいる。** **前提**：提出日時を初回提出日時・最終更新日時の2値で持つ（H-6）／締切が未設定の資料枠を許す（H-27）。**決着時に変わる箇所**：`submissions`・`summaries` の日時列（6-3）と本表の遅延の導出式。**未決 ID**：H-6・H-27。
