...
| **H-29** | **生徒とチームの所属関係が、どうやって成立するのかが未定義**（サインイン前の生徒を先生が割り当てられない） | **先行しない。** `../requirements.md` §3-8 を書く前に決着させる |
...
> **2026-09-21 追加：H-29 は ER の一部だけを止める。** 発表会・資料枠・提出・発表の関連は引ける。止まるのは**ユーザーとチームの所属関係**で、ここは「どう格納するか」の手前に「**どうやって成立するか**」が未定義な状態にある。`09-nfr.md` が文脈判定を「06 待ち」と書いているのはこの箇所だが、**ER を引いても答えは出ない**（`../open-questions.md` H-29）。
