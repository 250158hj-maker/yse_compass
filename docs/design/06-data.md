...
> **更新のしかた**：上書き
> **主担当**：蒲山
> **最終更新**：2026-09-21（水戸・**H-20 決着を反映**＝所属関係はチームのメンバー行で表す。氏名必須・利用者への参照は任意・**メールアドレスの列は持たない**。ER 直結の未決から H-20 を削除。`../decisions.md` 2026-09-21 の 2 行／**同日に ER 直結の未決へ H-20 を追加**＝所属関係の成立手段が未定義）／2026-09-04（水戸）
...
> **2026-09-21 更新：H-20 は同日決着したので、ER を止める要因ではなくなった。** 止まっていたのは**ユーザーとチームの所属関係**で、「どう格納するか」の手前に「**どうやって成立するか**」が未定義だった。**成立手段が決まったので、あとはこの章が引けば埋まる**（`../decisions.md` 2026-09-21 の 2 行・`../requirements.md` §3-8）。`09-nfr.md` が文脈判定を「06 待ち」と書いているのはこの箇所で、**決着前は ER を引いても答えが出ない状態だった**が、いまは 06 が引けば解消する。
