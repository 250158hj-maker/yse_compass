...
> **最終更新**：2026-09-17（蒲山・F-04 に追記＝OAuth疎通検証は校内PC・校内Wi-Fiから実施していたと判明。`open-questions.md` H-23 の論点(a)を部分的に裏付け）／2026-09-11（蒲山・F-04 を追記＝Auth.js × 学校Google WorkspaceのOAuth疎通検証）／2026-09-11（蒲山・F-03 を追記＝H-12 のスコープ検証）
...
- **追記（2026-09-17・蒲山）**：上記の検証は**学校内の PC・校内 Wi-Fi から実施していた**（steps 欄が「ローカル環境」とのみ書いており環境の所在が曖昧だったため明記）。`pnpm dev` のサーバープロセスもその PC 上で動いていたため、①生徒のブラウザ→Google の同意画面、②サーバープロセス→Google のトークンエンドポイント、の両方の通信がその教室の校内 Wi-Fi 経由で成立したことになる。したがって `open-questions.md` H-23 の判断が要る点 (a)（校内ネットワークから Google に到達できるか）は、**少なくとも本件で使った1台のPC・1つの教室のWi-Fiでは実証済み**。ただし①別の教室（各教室が独立Wi-Fiを持つ・H-23）でも同様に通るか、②本番サーバーが実際に置かれる場所（教室のWi-Fiと同じ扱いを受けるとは限らない）、の2点は未検証のまま残る。H-23 の残る論点はこの2点に絞り込める
