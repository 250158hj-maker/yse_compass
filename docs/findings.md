...
### F-06  学校配布メールアドレスのローカル部に規則性は無い（H-10 が賭けを避けた前提の検証）

- ref   : `open-questions.md` H-29（決着）／`decisions.md` 2026-09-05（H-10 決着・根拠①「生徒の学籍番号形式が安定している」という**未検証の前提**に賭けない）／`requirements.md` §3-6・§3-8
- steps : H-29（所属関係の成立手段）の検討で、「先生が 120 件のメールアドレスを DB へ入れる」以外の経路として**学籍番号からの生成**が候補に挙がった。学校配布の `@yse-c.net` アカウントの実物でローカル部の構成を確認した（2026-09-21・水戸）
- what  : **ローカル部の末尾は規則で導けない。** 実例 `250158hj@yse-c.net` の `hj` にあたる部分は**ランダム**で、学籍番号からも氏名からも生成できない。併せて確認できたのは、**ドメインが `yse-c.net` に固定**であることと、**学校が全員にアカウントを配布している**（＝全生徒が必ず 1 つ持つ）ことの 2 点
- why   : **これは論点ではなく、賭けの決着である。** 本欄の他の行と違って `why` に「両方の選択肢の理」が書けない（F-05 と同じ形）。`decisions.md` 2026-09-05（H-10）の根拠①は、メールアドレスのパターン判定を「学籍番号形式が安定している」という**未検証の前提に賭ける**として退けた。本件はその前提が**偽であることを確定させる** — **H-10 は賭けを避けた判断だったが、実際には賭けていれば負けていた。** 記録する理由は、**H-10 の文面が「賭けたくない」としか書いておらず「不可能」とは書いていない**ため。事実を残さないと、パターン方式は何度でも再提案される
- 決着  : **2026-09-21。** H-29 の決着（`decisions.md` 2026-09-21 の 2 行）へ反映済み。**メールアドレスを転記なしで得る経路は本人の OAuth サインインのみ**という結論の根拠になった。**H-10 の決定本体に変更は無い**（結論は同じで、根拠が 1 つ強くなっただけ）
