...
> **最終更新**：2026-09-25（蒲山・F-03 に追記＝「Internal use only」はGoogle公式ページの検証例外一覧に明記されているが、成立するのは「Cloudプロジェクトを組織が所有し、同意画面をInternalに設定した場合に限る」（Google審査と組織管理者承認は別の関門）。whatの「組織内限定であっても検証は免除されない」はこの条件下では訂正が要ると判明。PR #32 水戸レビューを受けて確認・PR #35 水戸レビューを反映）／2026-09-18（水戸・F-05 を追記＝モックの公開許可操作が正典と一致していない）／2026-09-11（蒲山・F-04 を追記＝Auth.js × 学校Google WorkspaceのOAuth疎通検証）／2026-09-11（蒲山・F-03 を追記＝H-12 のスコープ検証）
