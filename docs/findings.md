...
> **最終更新**：2026-09-25（蒲山・F-03 に実機確認結果を追記＝`drive.readonly`をInternal設定で要求し実際にサインインを試したところ、Google審査・組織管理者承認のどちらのブロックも発生せず通常の同意画面のみで完了。「Internal use only」はGoogle公式ページの検証例外一覧に明記されているが、成立するのは「Cloudプロジェクトを組織が所有し、同意画面をInternalに設定した場合に限る」（Google審査と組織管理者承認は別の関門）。whatの「組織内限定であっても検証は免除されない」はこの条件下では訂正が要ると判明。PR #32・#35 水戸レビューを反映）／2026-09-18（水戸・F-05 を追記＝モックの公開許可操作が正典と一致していない）／2026-09-11（蒲山・F-04 を追記＝Auth.js × 学校Google WorkspaceのOAuth疎通検証）／2026-09-11（蒲山・F-03 を追記＝H-12 のスコープ検証）
...
- 追記  : 2026-09-25（蒲山）。上記の実機での確認を行った。F-04 の OAuth スパイク（`src/auth.ts`）の認可リクエストに `https://www.googleapis.com/auth/drive.readonly` を一時的に追加し、`@yse-c.net` アカウントでサインインを試した（ローカル環境・`pnpm dev`）。
  ...
  **決着欄の「実機での確認が要る」はこれで解消。** 少なくとも現時点のこの Cloud プロジェクト・この Workspace 組織の設定では、`drive.readonly`（restricted scope）を Internal 設定で要求しても、Google の外部審査・組織管理者の承認のどちらも発生しないことが実機で確認できた。**ただしこれは今の管理者ポリシー設定での結果であり、瀬戸先生が今後ポリシーを変更すれば挙動が変わりうる。** また `drive.readonly` 止まりの確認であり、`drive`（読み書き）や他の restricted scope で同じ結果になるかは未確認
