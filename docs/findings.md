...
> **最終更新**：2026-09-25（蒲山・F-03 にDrive API呼び出しの実機確認結果を追記＝同意画面通過後、Drive APIを`GET .../files`で実際に呼び出し`200`を確認（`domainPolicy`エラー無し）。確認範囲を「蒲山（生徒）のアカウントでは」に限定し「先生のアカウントでは未確認」と明記。「Internal use only」はGoogle公式ページの検証例外一覧に明記されているが、成立するのは「Cloudプロジェクトを組織が所有し、同意画面をInternalに設定した場合に限る」（Google審査と組織管理者承認は別の関門）。PR #32・#35 水戸レビューを反映）／2026-09-18（水戸・F-05 を追記＝モックの公開許可操作が正典と一致していない）／2026-09-11（蒲山・F-04 を追記＝Auth.js × 学校Google WorkspaceのOAuth疎通検証）／2026-09-11（蒲山・F-03 を追記＝H-12 のスコープ検証）
