...
- steps : PR #25 で `design/07-interface.md` 7-3 を執筆した際（2026-09-11 13:51・`a802045`・蒲山）、Drive のディレクトリ構成を `glossary.md` §3 から本文へ書き写した。2026-09-21 の水戸のレビュー（PR #25 の A-2）が禁則2 の違反として指摘し、同日 `glossary.md` §3 への参照に置き換えた（`31f3a55`）
- what  : 書き写した列には、原文の「番号_発表名」の「番号_」が無かった。レビューはこれを**転記の劣化**と判定した。**ところが誤っていたのは原文の側だった。** 原文は 2026-08-30（`00f3eef`）から「番号_発表名」で、H-21 の決着（水戸が Drive の現物を直接照合・2026-09-11）で訂正されている。**ただし訂正は PR #23 にしか無く、main の原文は本項の起票時点でも訂正前のままである。** #23 の訂正コミット（`7494af4`・13:41）は、07 の書き写しの 10 分前だった。同じ原文から、#23 の初稿は `teams.number` を起こしてもいた（#23 の `decisions.md` 2026-09-11 行）。**Drive を照合した本人のレビューでも、判定の基準は main の原文だった**
- why   : **原文を正とする理**＝禁則2 の考え方では正典は原文の側にあり、食い違えば写しが古くなったと読むのが既定。**転記を正とする理**＝原文の訂正が未マージの PR に留まっている間は、main の原文のほうが古い。本件の訂正はリポジトリの中（#23 のブランチ）にあったが、main だけを読む人には存在しなかった。**どちらが正かは、原文と転記を読み比べても決まらない。** 決め手は、訂正を持つブランチの決定行か、リポジトリの外の現物（Drive）だった。**二重管理のコストは、写しが古くなることだけではない。正典の訂正が未マージの間に食い違いを正典へ合わせて直すと、正しい写しを誤りへ戻しかねない**
