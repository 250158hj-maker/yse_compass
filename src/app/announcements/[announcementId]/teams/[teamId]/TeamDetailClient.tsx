...
import type { Announcement, CommentLabel, SubmissionStatus, Submission, Team } from "@/lib/mock-data";
...
export default function TeamDetailClient({
  ...
  const canPostNewComment = role === "student2" || role === "student1";
  const canReply = role === "teacher" || role === "student2";
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <div className="mt-4 flex items-start justify-between">
        <div>
          <span className="text-xs font-medium text-slate-400">
            {team.className} ・ {announcement.title}
          </span>
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-medium ${statusStyle[submission.submissionStatus]}`}
        >
      ...
      <section className="mt-6 rounded-xl border border-slate-200 bg-white p-6">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-400">
            コメント・フィードバック({submission.comments.length})
          </h2>
          <span className="text-xs text-slate-400">♡ {submission.likeCount}</span>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4">
          ...
          {submission.comments.map((comment) => (
            <div key={comment.id} className="rounded-lg border border-slate-100 p-4">
              ...
              {canReply && (
                <div className="mt-3 border-l-2 border-transparent pl-4">
                  <input
                    type="text"
                    placeholder="返信を入力(モックのため送信はできません)"
                    className="w-full rounded-md border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400"
                  />
                </div>
              )}
            </div>
